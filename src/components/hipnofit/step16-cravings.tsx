...
type Props = {
  onContinue: (data: { cravings: string[] }) => void;
};

const options = [
  'Doces (chocolate, balas, etc.)', 'Salgados (batata frita, salgadinhos, etc.)', 'Carboidratos (pão, massa, etc.)',
  'Fast food', 'Desejos emocionais (estresse, tédio)', 'Outro'
];

export function Step16_Cravings({ onContinue }: Props) {
  ...
  return (
    <Card className="w-full max-w-lg mx-auto text-center shadow-lg border-none bg-transparent">
      <CardHeader>
        <CardTitle className="text-2xl font-bold font-headline">Quais desejos são mais difíceis de resistir?</CardTitle>
