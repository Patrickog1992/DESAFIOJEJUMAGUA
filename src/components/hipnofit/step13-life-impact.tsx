...
type Props = {
  onContinue: (data: { lifeImpact: string[] }) => void;
};

const options = [
  'Desconforto físico', 'Nervosismo ao socializar', 'Preocupação ao viajar',
  'Dificuldade no trabalho', 'Autoestima negativa', 'Não tenho certeza'
];

export function Step13_LifeImpact({ onContinue }: Props) {
  ...
  const handleContinue = () => {
    if (selected.length > 0) {
      onContinue({ lifeImpact: selected });
    }
  };
