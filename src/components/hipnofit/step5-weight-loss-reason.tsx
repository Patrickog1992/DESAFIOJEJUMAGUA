...
type Props = {
  onContinue: (data: { weightLossReason: string[] }) => void;
};

const options = [
  'Aumento da autoestima', 'Melhor aparência', 'Movimentação mais fácil',
  'Maior longevidade', 'Evento específico', 'Melhor qualidade de vida', 'Outro'
];

export function Step5_WeightLossReason({ onContinue }: Props) {
  ...
  const handleContinue = () => {
    if (selected.length > 0) {
      onContinue({ weightLossReason: selected });
    }
  };
  ...
  return (
    <Card className="w-full max-w-lg mx-auto text-center shadow-lg border-none bg-transparent">
      <CardHeader>
        <CardTitle className="text-2xl font-bold font-headline">Por que você quer perder peso?</CardTitle>
      </CardHeader>
